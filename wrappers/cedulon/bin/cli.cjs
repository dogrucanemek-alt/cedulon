...
// Launcher for `cedulon`. This package holds no logic of its own: it resolves
...
try {
  ...
  process.stderr.write("cedulon" + ': cannot resolve ' + PKG + '.\n')
  process.exit(1)
}
...
if (!relative) {
  process.stderr.write(
    "cedulon" + ': ' + PKG + '@' + targetPkg.version + ' does not provide ' + TARGET + '.\n'
  )
  process.exit(1)
}
...
if (result.error) {
  process.stderr.write("cedulon" + ': ' + result.error.message + '\n')
  process.exit(1)
}
