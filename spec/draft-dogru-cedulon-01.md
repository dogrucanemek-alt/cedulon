...
3. Check scope. Report each settlement record outside the declared
   ...
   (`MUST-T10-11`). If the verifier stated no period, emit
   `unstated-audit-window` and treat the guarantee as conditional
   (`MUST-T10-15`).
...
8. For each `ref` that appears exactly once on each side, require a
   ...
   `settled-without-ref`.
9. A `ref` reported as `duplicate-ref` MUST still be reconciled by
   ...
   MUST still report findings for the remaining records rather than
   abandon the audit.
...
14. If two successfully verified checkpoints share an epoch number
    ...
    (`MUST-T11-3`).
15. If any fail-severity finding exists, the audit MUST fail
    (`MUST-T10-4`).
...
| unstated-audit-window | warn | The verifier stated no period, so the extract defined its own; guarantee is conditional |
...
Maturity:
: Research code by a single implementer. Interoperability with an
  independent implementation has not been demonstrated; no second
  implementation is known to the author.
...
Experience:
: Two independent readers ran an earlier revision and reported defects,
...
Three changes are not repairs and are recorded here so a reader of -00
is not surprised:

- A failed extract signature was a warning in -00 and stays one when no
  ...
  clarification.
- Requiring that a disagreeing caller-supplied list be *reported* as
...
- -00 said reversal, refund, and partial settlement were "expected in
  ...
  this revision and expected later, with no date.
...
this document adopts, and is also the author of {{SCHROCK}}, cited here
...
this text, and any error in it is the author's.
