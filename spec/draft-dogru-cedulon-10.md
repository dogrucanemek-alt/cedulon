...
verifier did not obtain (`MUST-T10-8`). -02 already
...
: `kid` identifies the verification key. This -10 does not specify
...
: See {{security}} of this document. A checkpoint carries the
  ...
  weight depends on a pinned issuer key ({{issuer-root}}) and never on
...
: A companion implementation with a runnable verification suite at
  ...
  of this document, not a second specification. This -10 is not an
...
: The receipt, checkpoint, extract, reconciliation and verification
  ...
  redaction. Every requirement added since -03 is covered by a
  ...
  proofs; tier 2 of {{witness}} is exercised against it red-then-green,
...
Experience:
: Every revision has been driven by what a reader found rather than by
  a plan: -01 and -02 by defects reported against their predecessors,
  -03 by a question about the pinned witness key that the verification
  algorithm did not answer, -04 by a first-failure list filed against
  the posted -03 by a reader who ran the {{vectors}} vectors against
  the archive bytes before reading the text, -05 by a
  canonicalization reading taken from a neighbouring draft, and -06
  and -07 by readers implementing from the posted text. T12 came from
  none of them: it was found while writing an adversarial task, in the
  ordering the implementation itself used, which produced against the
  issuer the one condition this document exists to make detectable.
...
Note on distribution: everything -03 through -07 added is in the
...
This revision changes no requirement, adds none and removes none. It
is a length pass, made because the document had reached 107 pages and
a reader who reviews for a living said plainly that nobody reads a
draft that long. The text is 92 pages.

The change log is the larger part of it. -09 carried a section for
every revision back to -01, 596 lines of prose about repairs whose
result is already stated where the requirement lives. Those sections
are replaced by {{changes-earlier}}, one paragraph naming the subject
of each revision, and by the Datatracker record {{CEDULON-DT}}, which
holds the text of every revision and the diff between any two. The
change notes for -09 and -08 are kept, because a reader implementing
from this text is most likely to have read one of those two.

{{impl-status}} is restated in the fields {{RFC7942}} asks for:
implementation, maturity, coverage, licensing, contact and
experience. What it loses is the per-reader narrative - which reader
reported which defect, in which revision, and which repair was found
inside an earlier repair. That account is not deleted; it moves to
STATUS.md in the repository, where it can be corrected without a
...
This revision changes no requirement. It is the editorial pass an
experienced reader asked for, and it adds two adjacent documents.

The abstract is two paragraphs. It states what the document
...
made once in {{evolution}}, where direction is discussed.

{{adjacent}} gains two paragraphs, on the agent auditing architecture
{{KUEHLEWIND-AUDIT}} and the verifiable agent conversation record
{{BIRKHOLZ-VAC}}, the two documents behind the AUDIT BoF request,
and says what each is beside this one. The sentences that name the
revision are renumbered, and no other text moved.

## Earlier revisions {#changes-earlier}

The revisions before -08 are on record in the Datatracker, where the
text of each and the diff between any two can be read
({{CEDULON-DT}}). In one line each. -07 widened `MUST-T10-19` from the
two surfaces -06 named to every structure an implementation returns
for an audit, and named the refusal of an extract whose window does
not end after it starts. -06 stated the wire form of the witness
receipt, which every earlier revision had described and none had
encoded, and added the requirements on the audit's declared scope over
the rail path. -05 added two decoder rules, each the named refusal of
a surface -04 left implicit, and turned the placeholder table of its
IANA section into the registration requests {{RFC6838}} asks for. -04
answered a first-failure list filed against the posted -03 by an
independent reader who ran the {{vectors}} vectors against the archive
bytes before reading the text. -03 closed eighteen decision points
that two readers working from -02's words alone could not resolve the
same way twice, five of them the same defect in different clothes.
-02 wired the checkpoint into the anchoring and verification path that
discharges the T11 guarantee, and added T12.

No requirement in this document rests on a sentence that lives only in
one of those change notes. Each requirement is stated where it
applies, carries its own identifier, and appears in the requirements
table; a reader who never opens an earlier revision can implement this
one.
...
threat-model MUST lines for them belong in later revisions (-11 or later), written with the same discipline as this -10.
...
Payment is the special case that this -10 implements. The same
...
Table 3, which -04 repaired - see {{changes-earlier}}), manifestHash=null,
