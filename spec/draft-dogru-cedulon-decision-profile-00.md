...
docname: draft-dogru-cedulon-decision-profile-00
date: 2026-09-04
category: info
submissiontype: IETF
ipr: trust200902
...
carries the profile prepared and unpublished.
...
All twelve labels are always present; a nullable claim carries CBOR
...
chain is the equivocation control of this profile, and a verifier MUST
...
dropped from the population without a word (`MUST-DP-5`).
...
the companion exported a `+cbor` name for it that nothing used; the
name is withdrawn with this revision. Section 9.3 of {{CEDULON}}
applies unchanged: a
...
- an allow and a row bind when the row's `effectHash` equals the
  record's `effectHash`; a difference is `effect-mismatch` (the
  content that occurred is not the content that was allowed);
...
states for the core's codes. Four are new to this profile:
...
The Decider signs two decisions for one request, or presents a record
twice, and offers each to a different reader. The chain
(`MUST-DP-5`) makes the second record unlinkable: it names the same
...
close the other route.

## D6: The class of the effect is not bound

A Decision Record names a reference and a content hash and does not
...
references are shared across classes does not, and SHOULD say so in
...
({{extract-auth}}). Registration in the standards tree requires IETF
...
The claim labels this document assigns, `-70501` through `-70512`
