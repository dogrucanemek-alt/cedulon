...
abbrev: Cedulon
docname: draft-dogru-cedulon-09
date: 2026-09-06
category: info
submissiontype: IETF
ipr: trust200902
...
: `kid` identifies the verification key. This -09 does not specify
...
: The receipt, checkpoint, extract, reconciliation, and verification
  ...
  the widening this revision makes to `MUST-T10-19`, are implemented
...
of a non-empty unprotected header (`MUST-T4-21`) - and what -07
...
requirement now names them all.

What the requirement asks of a structure computed with no extract is
nothing: there is no declared population, and the structure names
none. The companion's MCP `cedulon_audit` result carries `scope`
...
extract rather than reconciling them, on the reasoning `MUST-T10-20`
...
threat-model MUST lines for them belong in later revisions (-10 or later), written with the same discipline as this -09.
...
Payment is the special case that this -09 implements. The same
