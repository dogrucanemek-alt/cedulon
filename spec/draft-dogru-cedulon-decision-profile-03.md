...
docname: draft-dogru-cedulon-decision-profile-03
date: 2026-09-06
category: info
submissiontype: IETF
ipr: trust200902
...
informative:
  ...
  B7N0DE:
    title: "Mapping 4: the Cedulon Decision Record and Effect Extract against decision-receipt and action-outcome (proofbundle)"
    author:
      - name: Konrad Gruszka
    date: 2026-09-04
    target: https://github.com/b7n0de/proofbundle/blob/main/docs/SCITT_CPB_MAPPING.md
  OUTCOME:
...
that path, not the whole of it. {{OUTCOME}} reconciles the exact
...
## The boundary rule {#boundary-rule}

One rule runs under several choices in this document, and it is
...
The rule was learned three times, in three shapes. A consistency
...
({{binding}}). The first two are the state form of the rule; the
third is the text form.

The consequence for this text is that a silence is stated as a
...
implementation finds a design note rather than a finding.
...
7 of {{CEDULON}} when the request is a JSON document, and its
UTF-8 octets when it is text. The request's fields are not fixed by
this document, and a deployment MUST state what it hashes.
`policyHash` is
...
clock-skew allowance of a window edge is `boundary-deferred`, and a
closing-edge allow whose `ref` the following extract names is
carried, not a finding. The Effect Extract declares no allowance of
its own ({{extract-schema}}); the verifier applies the core's default,
which the companion holds at five minutes.

The binding does not order the two clocks. A row's `timestampMs` is
checked against the window it sits in and not against the
`timestampMs` of the record it answers, so a row dated before the
...
is the text form of the rule in {{boundary-rule}}: the silence costs
a sentence here, and would cost a finding later.
...
A deployment MUST state which of the two it has, and a verifier MUST
treat the guarantee as conditional where the extract root and the
decider root are, or may be, the same party (`MUST-DP-9`). The
...
The shape of the threat is not particular to this profile. A reader
...
because the verdict is not read on the outcome path {{B7N0DE}}. That
is D1 as a verifying pair rather than a finding, recorded by the
reader as a measurement of that artefact and cited here as one.
...
Maturity:
: Published. The code carrying -01's claim set, the class under the
...
: Read by a second reader: one frozen fixture, a leaked refusal in
  ...
  {{OUTCOME}} for that document's verifier, with the two results
  recorded side by side the next day. This reader reported
  `effect-against-refusal` on the leaked row; the other reported the
  ...
  says where the refusal failed.

: Reproduced: the companion's suite at the commit that posted -00 was
  ...
  an effect dated before its decision, which is what {{binding}} now
...
- The rule under three of this document's choices is stated once, as
  {{boundary-rule}}: a state that never crosses the boundary does not
  ...
  learned are named there, and the unordered clocks of {{binding}}
  now point to it. No rule, claim, or finding changes.

# Changes from -01
{:numbered="false"}

- The binding does not order the two clocks: a row dated before the
  record it answers binds as if it had followed it, on this profile
  and on spend alike. Stated in {{binding}}, with no finding named;
  the Effect Extract's lack of a clock-skew member and the default the
  verifier applies are stated beside it.
- Two boundary sentences corrected: {{AEB}} runs from invocation to an
  authenticated reconciliation and is not only what precedes an
  effect; the observation in {{OUTCOME}} is independent where the
  relying party chose an independent source, not by construction.
- {{impl-status}} records the companion as published at 0.13.0, one
  fixture read by a second reader with the limits of that run, and two
  reproductions of the suite.
- D1 cites a measurement of the same threat on a second evidence
  format, whose verifier accepts a refusal beside an executed outcome.
- Two sentences tightened after a reader's mapping: the request
  encoding is fixed and the request's fields are not ({{record-labels}});
  `MUST-DP-9` is the statement and the downgrade, not a demand for
  independence ({{roots}}).
...
Pablo Etcheverry ran the suite at the posted -00 commit and asked
three questions of it; the unordered clocks of {{binding}} are his
...
fixture with an adapter written for that document's verifier.

The rule of {{boundary-rule}} was named as one rule by Henri
Sirkkavaara, after he found and closed its state form in Vaara's own
consistency checker and read the unordered clocks of {{binding}} as
...
how the section describes them.
